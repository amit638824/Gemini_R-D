...
export class TimerScheduler {
  ...
  private handler: ReengageHandler | null = null;
  ...
  extendOrSet(input: {
    ...
    let timer =
      (input.timerId && timers.find((t) => t.id === input.timerId)) ||
      timers.find((t) =>
        input.label
    ...
    if (!timer) {
      const duration =
        input.setRemainingSec ?? input.addSec ?? env.timerWarnBeforeSec;
      return this.create({
    ...
    const remaining = Math.max(
      0,
      Math.round((Date.parse(timer.endsAt) - Date.now()) / 1000),
    );
    ...
    const updated: TimerState = {
      ...
      warned: false,
      fired: false,
      warnBeforeSec: Math.min(
        timer.warnBeforeSec,
        Math.max(0, nextRemaining - 1),
      ),
    };
  ...
  cancel(timerId?: string, label?: string): boolean {
    const timers = stateStore.get().timers.filter((t) => !t.fired);
    const target =
      (timerId && timers.find((t) => t.id === timerId)) ||
      (label &&
    ...
    for (const timer of timers) {
      ...
      if (!timer.warned && remainingSec <= timer.warnBeforeSec && remainingSec > 0) {
        ...
        await this.handler({
          ...
          message:
            `PROACTIVE RE-ENGAGE NOW. Speak briefly to the user without waiting: ` +
